...
router.delete("/:cartId/products/:prodId", async (req, res) => {
  ...
  try {
    ...
    const product = await ProductDao.getById(Number(prodId));
    if (!product) throw new Error("wrongProductId");

    const products = cart.products;
    const check = products.find((item) => item.id === Number(prodId));
    ...
    const updatedCart = await CartDao.updateById(Number(cartId), newCart);
